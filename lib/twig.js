...
const options = { xmlns: true, trim: true };

var saxStream = require("sax").createStream(true, { position: true,xmlns: true , trim: true});
...
saxStream.on("text", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("processinginstruction", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("sgmldeclaration", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("doctype", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("comment", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("opentagstart", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("attribute", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("closetag", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})

saxStream.on("ready", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})
saxStream.on("end", function (node, a, b, v) {
   // same object as above
   console.log('In node');
})

fs.createReadStream("samples/contact-info.xml").pipe(saxStream);
...
class Elt {
    ...
    * @param {?string|RegExp|object|function} cond - If `undefined`, then all attributes are returned.<br> 
    * If `string` or `RegExp` then attributes matching this string are returned.<br>  
